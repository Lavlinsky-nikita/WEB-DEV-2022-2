...
def convert_config_to_dict(config_filename):
    ...
    with open(config_filename) as f:
        for line in f:
            line = line.rstrip()
            # Если строчка не начинается с ! и не содежратся из словаря игнор
            if line and not (line.startswith("!") or ignore_command(line, ignore)):
                # isalnum позволяет проверить, состоит ли строка из букв или цифр
                if line[0].isalnum():
                    config_dict[line] = []
                else:
                    config_dict[line].append(line.strip())
    return config_dict

x = convert_config_to_dict("config_sw1.txt")
print(x)